export const appbardata = {
    sitename:'Welfare Bank System (EC-WBS)', 
    search:'Search',
    dashboard:'Dashboard',
    logout:"logout"
};

export const Apidata = {
    api:'http://127.0.0.1:5053/'
}

export const searchdata = {
    res_id:'Postal ID',
    res_date:'Received Date',
    res_type:'Postal Type',
    res_title:'Letter Title',
    res_sender:'Sender',
    search:'Search',
    advanced_search:"Advanced Search"
}

export const loginformdata={
    ec:'Election Commission of Sri Lanka',
    pms:'Postal Management System',
    ecpms:'Postal Management System (EC-PMS) ',
    title:'Sign in ',
    username:'User Name',
    password:'Password',
    login:' Sign In',
    username_required:'User Name required',
    password_required:'Password required',
    password_min:'Password must be at least 6 characters',
    new_postal:'New Postal',
    search:'Search'

}

export const Tabsdata ={
    letter:'Letter',
    cheque:'Cheque',
    email:'Email',
    fax:'Fax'
}

export const LetterFormdata ={
    depositer_name:"Officer Name",
    amt:"Amount",
    bank_date:"Bank Date",
    bank_title:"Bank Transaction",
    loan_paid_info:"Loan paid info",
    loan_pay_plan:"Loan Pay plan",
    loan_payment:"Loan Payment",
    get_details:"Get Details",
    tran_loan_title:"Loan Transaction",
    loan_installmet:"Loan Installment",
    member_fee:"Member Fee",
    diposit_fee:"Diposit / Withdraw",
    year:"Year",
    month:"Month",
    total:"Total",
    diposit_withdrow:"Diposit/Withdraw",
    member_fee:"Member Fee",
    pay_plan:"Payment Plan",
    loan_date:"Loan Date",
    loan_id:"Loan Id",
    loan_pay_detail:"Loan Payment Details",
    get:"Get",
    loan_detail:"Loan Details",
    Customer_reg_fee:"Registration Fee",
    loan_tran_title:"Loan Transaction",
    loan_id:"Loan Id",
    debit:"Debit",
    credit:"Credit",
    comment:"Comment",
    tran_title:"Transaction",
    tran_amt:"Amount",
    loan_comment:"Loan Comment",
    calculate:"Calculate",
    loan_title:"Create New Loan",
    monthly_installment:"Monthly Installment",
    loan_full_interest:"Loan Full Interest",
    loan_period:"Periods",
    loan_interest:"Interest Rate",
    loan_type:"Loan Type",
    loan_amt:"Loan Amount",
    acc_title:"Create New Account",
    Customer_job:"Customer Job Title",
    Customer_branch:"Customer Branch",
    customer_id:"Customer Id",
    customer_name:"Customer Name",
    close:'Close',
    colse_postal:'Close Postal',
    add_comment:'Add Comment',
    forward_to:'Forward To',
    forward:'Forward',
    registered_post:'Registered Post',
    normal_post:'Normal Post',
    byhand_post:'By Hand',
    secret:'Secret Letter',
    not_secret:'Not Secret Letter',
    past_ref:'Past reference',
    reg_number:'Registred Post Number',
    postoffice:'Post Office Name',
    posted_date:'Posted Date',
    letter_date:'Letter Date',
    recived_date:'Recived Date',
    sender_name:'Sender Name',
    sender_address:'Sender Address',
    receiver_name:'Receiver Name',
    receiver_address:'Receiver Address',
    upload_file:'Upload File',
    title:'Enter Letter Details',
    cheque_title:'Enter Cheque Details',
    letter_cheque_title:'Title',
    letter_title:'Letter Title',
    attachments:'Attachmenets',
    sender_email:'Sender Email',
    sender_fax:'Sender Fax Number',
    cheque_no:'Cheque Number',
    cheque_amt:'Cheque Amount',
    cheque_bank:'Bank',
    save:"Save",
    finish: 'Finish',
    letter_reg_number:'Postal Id',
    upload_file:'Upload File',
    drop_item_here:'Drop image here',
    preview:'Preview',
    assign:"Assign",
    postaldetails:"Postal Details",
    postalimage:"Postal Image",
    comment:"Comment",
    comments:"Comments",
    postal_type:"Postal Type",
    secret:"Secret",
    res_mode:"Received Mode",
    reg_date:'Registered Date'
}

export const Aodata = {
    new_postals:'New Postals'
}

export const Cgdata = {
    new_postals:'New Postals',
    opened_postals:'Opened Postals',
    closed_postals:'Closed Postals'
}

export const Sodata = {
    new_postals:'New Postals',
    open_postals:'Opened Postals',
    closed_postals:'Closed Postals'
}

export const Admindata = {
    new_user:'New User',
    new_path:'Opened Path',
}

